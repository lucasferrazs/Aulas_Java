...
Ela é responsável por inicializar o aplicativo e fornecer um ponto de entrada para o ciclo de vida do aplicativo.
A classe Application é abstrata e deve ser estendida por uma classe concreta que implemente o método start(Stage primaryStage).
O método start é chamado quando o aplicativo é iniciado e é responsável por criar a interface gráfica do aplicativo.
Stage

A classe Stage representa a janela principal do aplicativo.
É o contêiner mais alto nível que contém a cena e os nodes.
Um aplicativo JavaFX pode ter múltiplos stages, mas apenas um pode ser o stage primário.
O stage é responsável por gerenciar a vida útil da cena e dos nodes.
...
É o contêiner que contém os nodes que compõem a interface gráfica do aplicativo.
A cena é responsável por gerenciar a layout e a renderização dos nodes.
...
É o elemento mais básico que compõe a cena e pode ser um controle, um gráfico, uma imagem, um texto, etc.
...
Um aplicativo JavaFX tem uma instância de Application que é o ponto de entrada do aplicativo.
A instância de Application cria um Stage primário que é a janela principal do aplicativo.
...
Suponha que você queira criar um aplicativo JavaFX que exiba uma janela com um botão e um label. A anatomia do aplicativo seria a seguinte:

Application: a classe que estende Application e implementa o método start(Stage primaryStage).
